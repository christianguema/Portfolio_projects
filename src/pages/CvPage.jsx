  ...
  return (
    <section className="max-w-4xl mx-auto px-6 py-24">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-12">
        <div>
          <p className="font-mono text-accent-1 text-sm mb-3">{"$ cat resume.txt"}</p>
          <h1 className="font-display text-3xl text-text mb-2">{identity.name}</h1>
      ...
      <div className="border border-accent-2/30 rounded-2xl p-8 mb-10">
        <h2 className="font-display text-xl text-accent-2 mb-8">Expérience</h2>
        <div className="flex flex-col gap-10">
          {experience.map((exp, i) => (
            <div key={i}>
              <div className="flex flex-wrap items-baseline justify-between gap-2 mb-2">
                <h3 className="font-display text-lg text-text">{exp.role}</h3>
                <span className="font-mono text-xs text-accent-1">{exp.period}</span>
              </div>
        ...
        <div className="flex flex-col gap-6">
          {education.map((edu, i) => (
            <div key={i} className="flex flex-wrap items-baseline justify-between gap-2">
              <div>
                <h3 className="font-display text-base text-text">{edu.degree}</h3>
                <p className="font-body text-sm text-text/70">{edu.school}</p>
              </div>
              <span className="font-mono text-xs text-accent-1">{edu.period}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="border border-accent-2/30 rounded-2xl p-8">
          <h2 className="font-display text-xl text-accent-2 mb-6">Compétences</h2>
